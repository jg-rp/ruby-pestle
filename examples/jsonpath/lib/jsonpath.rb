...
module JSONPathPest
  ...
  FUNCTION_EXTENSIONS = {
    "length" => LengthFunctionExtension.new,
    "count" => CountFunctionExtension.new,
    "value" => ValueFunctionExtension.new,
    "match" => MatchFunctionExtension.new,
    "search" => SearchFunctionExtension.new
  }.freeze
  ...
  def self.parse(query)
    ...
    pairs.each do |pair|
      ...
      in :EOI, _
        ...
        raise "expected a segment, found #{pair.rule} -> #{pair.text.inspect}"
      end
    end
  ...
  def self.parse_test_expression(pair)
    ...
    in :function_expr, [name, *rest]
      ...
      assert_well_typed(func_name, func, args)
  ...
  def self.assert_well_typed(func_name, func_args, func)
    ...
    func.class::ARG_TYPES.each_with_index do |typ, idx|
      arg = func_args[idx]

      case typ
      when :value_expression
        unless arg.class.to_s.end_with?("Literal") ||
               (arg.respond_to?(:query) && arg.query.singular?) ||
               function_return_type(arg) == :value_expression
          raise "#{func_name}() argument #{idx} must be of ValueType"
        end
      when :logical_expression
        unless arg.is_a?(RootQueryExpression) || arg.is_a?(RelativeQueryExpression) ||
               arg.is_a?(LogicalAndExpression) || arg.is_a?(LogicalOrExpression) ||
               arg.is_a?(EqExpression) || arg.is_a?(NqExpression) || arg.is_a?(LeExpression) ||
               arg.is_a?(GeExpression) || arg.is_a?(LtExpression) || arg.is_a?(GtExpression)
          raise "#{func_name}() argument #{idx} must be of LogicalType"
        end

      when :nodes_expression
        unless arg.is_a?(RootQueryExpression) || arg.is_a?(RelativeQueryExpression) ||
               function_return_type(arg) == :nodes_expression
          raise "#{func_name}() argument #{idx} must be of NodesType"
        end
      end
    end
  end

  def self.function_return_type(expr)
    return nil unless expr.is_a?(FunctionExpression)

    func = self::FUNCTION_EXTENSIONS[expr.name]
    func.class::RETURN_TYPE
  end

  def self.assert_compared(expr)
    if expr.is_a?(FunctionExpression) && expr.func.class::RETURN_TYPE == :value_expression
      raise "result of #{expr.name} must be compared"
    end
  end
